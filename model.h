...
public:
	...
	void LoadMaterials() {
		...
		for (size_t i = 0; i < model.materials.size(); ++i) {
			...
			if (material.name == "Material.001" || material.name == "Material.002" || material.name == "Material.003" || material.name == "Material.004" || material.name == "Material.005" || material.name == "Material.006") {
				metallicRoughness = make_shared<SpectrumConstantTexture>(vec3(0, 0, 0));
				materials[i] = make_shared<DielectricMaterial>(metallicRoughness, make_shared<ConstantSpectrum>(1.45));
			}
